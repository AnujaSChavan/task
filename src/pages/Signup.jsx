...
export default function Signup() {
	...
	async function onSubmit(e) {
		...
		try {
			await signup(undefined, email, password)
			navigate('/app')
		} catch (err) {
				...
				<div className="w-full max-w-[520px]">
					<div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl p-8">
						<div className="text-center mb-6">
							...
							<p className="text-slate-500 dark:text-slate-400">Start organizing your tasks</p>
						</div>
						...
						<div className="text-center mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
							<p className="text-slate-600 dark:text-slate-400">Already have an account? <Link to="/login" className="text-blue-600 hover:text-blue-700 font-semibold">Sign in</Link></p>
						</div>
					</div>
				</div>
			</main>
